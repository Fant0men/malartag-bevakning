...
def get_arrivals(
    ...
    """
    Hämtar ankommande tåg till en station och behåller bara de som finns
    med i valid_names (dvs. bekräftat hör till vår linje). Om search_date/
    search_time anges (t.ex. några timmar bakåt i tiden) hämtas istället
    historiska ankomster - praktiskt för att verifiera att förseningar
    faktiskt fångas upp korrekt, genom att kolla tåg som redan hunnit fram.
    """
    params = {
        "id": station_id,
        "format": "json",
    ...
    if not arrivals:
        print(
            f"DEBUG: inga ankomster alls returnerades för {station_name} "
            f"(sökparametrar: date={params.get('date')}, time={params.get('time')}). "
            f"Rått svar (första 500 tecken): {json.dumps(data, ensure_ascii=False)[:500]}"
        )
...
def main() -> None:
    ...
    # Historik-läge: om HISTORY_HOURS_BACK är satt, kolla tåg som redan
    # anlänt för X timmar sedan istället för kommande/pågående tåg. Bra för
    # att verifiera att förseningslogiken fungerar, utan att behöva vänta
    # på att en riktig försening ska inträffa live.
    ...
    if history_hours:
        ...
        print(f"HISTORIK-LÄGE: kollar ankomster från och med {search_date} {search_time}")

    # Hämta ankomster till båda stationerna, filtrerat mot giltiga tågnummer.
    arrivals_a = get_arrivals(station_a_id, STATION_A, valid_names, search_date, search_time)
    arrivals_b = get_arrivals(station_b_id, STATION_B, valid_names, search_date, search_time)
    ...
    if not history_hours:
        # I historik-läge sparar vi inget state och skickar inga vanliga
        # förseningsnotiser - det är bara till för att titta, inte bevaka.
