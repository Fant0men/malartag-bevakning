...
def run_daily_summary(sig_a: str, sig_b: str) -> None:
    ...
    (docs_dir / "index.html").write_text(
        build_html_report(rows, report_date, css_href="style.css", nav_html=today_nav, extra_sections=extra_sections),
        encoding="utf-8",
    )

    # Samma rapport arkiverad under sitt datum, för historik.
    (archive_dir / f"{report_date}.html").write_text(
        build_html_report(rows, report_date, css_href="../style.css", nav_html=archive_nav, extra_sections=extra_sections),
        encoding="utf-8",
    )
