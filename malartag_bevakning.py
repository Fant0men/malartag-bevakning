...
def get_full_journey_stop_names(dep: dict) -> list[str]:
    ...
    if ref.startswith("http"):
        ...
        # I andra fall (vanligast) är ref bara en kort kod som ska skickas
        # som query-parameter till journeyDetail-ändpunkten.
        url = f"{RESROBOT_BASE}/journeyDetail"
        params = {"ref": ref, "format": "json", "accessId": RESROBOT_API_KEY}
...
def get_departures(station_id: str, other_station_name: str) -> list[dict]:
    ...
    for dep in departures:
        ...
        operator = product.get("operator", "") or product.get("name", "")
        if "mälartåg" not in operator.lower():
            continue
