...
# --------------------------------------------------------------------------
def build_html_report(rows: list[dict], report_date: str) -> str:
    """Bygger en enkel, självständig HTML-sida med dagens fullständiga resultat."""
    table_rows = ""
    for r in rows:
...
<style>
  ...
  tr.delayed {{ color: #ffb84d; }}
  tr.cancelled {{ color: #ff6b6b; font-weight: bold; }}
  tr.ontime {{ color: #8fd694; }}
  .meta {{ color: #999; font-size: 0.9rem; }}
</style>
</head>
<body>
  <h1>Mälartåg-bevakning: Södertälje Syd ↔ Eskilstuna C</h1>
  <p class="meta">Rapport för {report_date}, senast uppdaterad automatiskt.</p>
  <table>
...
def run_daily_summary(sig_a: str, sig_b: str) -> None:
    ...
    (docs_dir / "index.html").write_text(build_html_report(rows, report_date), encoding="utf-8")
