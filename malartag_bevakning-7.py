...
Datakälla: Trafiklab ResRobot Timetables API v2.1 (https://www.trafiklab.se),
           reseplanerar-ändpunkten 'trip'. Den här ändpunkten anger
           start- och slutstation direkt (originId/destId) och returnerar
           bara resor som verkligen går mellan just de två stationerna -
           det behövs eftersom Mälartåg kör FLERA olika linjer (bl.a. även
           Stockholm-Norrköping via Södertälje Syd, som INTE går via
           Eskilstuna), så ett rent operatörsfilter räcker inte.
...
Känd begränsning: eftersom 'trip' bara söker framåt i tiden (en bakåt-sökning
gav 400 Bad Request från API:et) kan ett tåg som redan avgått från sin
startstation men inte hunnit fram till slutstationen tillfälligt falla
bort ur bevakningen under själva resan. I praktiken hinner du redan ha
fått en notis om förseningen innan tåget avgick, eftersom skriptet körs
var 15:e minut och då redan sett tåget som en kommande avgång.
"""
...
def get_direct_trips(origin_id: str, dest_id: str, origin_name: str, dest_name: str) -> list[dict]:
    ...
    via ResRobots reseplanerare ('trip'-ändpunkten). Detta garanterar att
    tåget verkligen går mellan just de här två stationerna - till skillnad
    från att bara filtrera på operatörsnamn, vilket även skulle fånga upp
    andra Mälartåg-linjer som inte alls går via Eskilstuna.
    """
    resp = requests.get(
